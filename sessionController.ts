...
export async function startAllSessions(
  ...
  if (secretkey === undefined) {
    tokenDecrypt = (token as any)?.split(' ')[1] ?? '';
  } else {
    tokenDecrypt = secretkey;
  }

  console.log('--- DEBUG TOKEN ---');
  console.log('secretkey param:', secretkey);
  console.log('authorization header:', token);
  console.log('tokenDecrypt:', tokenDecrypt);
  console.log('serverOptions.secretKey:', req.serverOptions.secretKey);
  console.log('env SECRET_KEY:', process.env.SECRET_KEY);
